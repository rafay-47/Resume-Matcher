...
class ResumeService:
    ...
    async def convert_and_store_resume(
        ...
        try:
            logging.debug("Converting resume file...")
    ...
    async def _extract_and_store_structured_resume(
        ...
        """
        try:
            structured_resume_model = await self._extract_structured_json(resume_text)
            if not structured_resume_model:
            ...
            structured_resume = structured_resume_model.model_dump()
            ...
            processed_resume = ProcessedResume(
                ...
                else json.dumps({"extracted_keywords": []}),
            )
    ...
    async def _extract_structured_json(
        ...
        try:
            ...
            logger.info(f"Raw LLM output for resume: {raw_output}")
            
            if not raw_output:
                logger.error("LLM returned empty response for resume")
                return None

            structured_resume: StructuredResumeModel = (
                StructuredResumeModel.model_validate(raw_output)
            )
            logger.info(f"Successfully validated structured resume: {structured_resume}")
            return structured_resume
            
        except ValidationError as e:
            logger.error(f"Resume validation error: {e}")
            logger.error(f"Raw output that failed validation: {raw_output}")
            error_details = []
            ...
            logger.error(f"Unexpected error during resume LLM processing: {e}")
            return None
