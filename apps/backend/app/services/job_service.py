...
class JobService:
    ...
    async def create_and_store_job(self, job_data: dict) -> List[str]:
        ...
        for job_description in job_data.get("job_descriptions", []):
            ...
            # Only add job_id to successful list if processing succeeds
            extraction_result = await self._extract_and_store_structured_job(
                job_id=job_id, job_description_text=job_description
            )
            
            if extraction_result is None:
                logger.error(f"Failed to process job description for job_id: {job_id}")
                # Remove the job from the session since processing failed
                self.db.expunge(job)
                continue
    ...
    async def _extract_structured_json(
        ...
        try:
            ...
            # Try to fix common validation issues and retry
            fixed_output = self._fix_common_validation_issues(raw_output)
            if fixed_output != raw_output:
                logger.info(f"Attempting validation with fixed output: {fixed_output}")
                try:
                    structured_job: StructuredJobModel = StructuredJobModel.model_validate(
                        fixed_output
                    )
                    logger.info("Validation succeeded after fixing common issues")
    ...
    def _fix_common_validation_issues(self, raw_output: Dict[str, Any]) -> Dict[str, Any]:
        """
        Fix common validation issues in LLM output before Pydantic validation.
        Returns a modified copy of the output with fixes applied.
        """
        import copy
        fixed_output = copy.deepcopy(raw_output)
        
        # Fix remoteStatus enum values
        if "location" in fixed_output and isinstance(fixed_output["location"], dict):
            location = fixed_output["location"]
            if "remoteStatus" in location:
                remote_status = location["remoteStatus"]
                if isinstance(remote_status, str):
                    # Map common variations to valid enum values
                    remote_status_lower = remote_status.lower()
                    if "flexible" in remote_status_lower or "hybrid" in remote_status_lower:
                        location["remoteStatus"] = "Hybrid"
                    elif "fully remote" in remote_status_lower or remote_status_lower == "remote":
                        location["remoteStatus"] = "Fully Remote"
                    elif "on-site" in remote_status_lower or "onsite" in remote_status_lower:
                        location["remoteStatus"] = "On-site"
                    elif "multiple" in remote_status_lower:
                        location["remoteStatus"] = "Multiple Locations"
                    else:
                        location["remoteStatus"] = "Not Specified"
        
        # Fix employmentType enum values
        if "employmentType" in fixed_output:
            employment_type = fixed_output["employmentType"]
            if isinstance(employment_type, str):
                employment_type_lower = employment_type.lower()
                if "full" in employment_type_lower:
                    fixed_output["employmentType"] = "Full-time"
                elif "part" in employment_type_lower:
                    fixed_output["employmentType"] = "Part-time"
                elif "contract" in employment_type_lower:
                    fixed_output["employmentType"] = "Contract"
                elif "intern" in employment_type_lower:
                    fixed_output["employmentType"] = "Internship"
                elif "temp" in employment_type_lower:
                    fixed_output["employmentType"] = "Temporary"
                else:
                    fixed_output["employmentType"] = "Not Specified"
        
        # Ensure required fields have default values if missing
        if "extractedKeywords" not in fixed_output or not fixed_output["extractedKeywords"]:
            fixed_output["extractedKeywords"] = []
