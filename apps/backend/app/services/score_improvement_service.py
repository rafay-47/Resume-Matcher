...
class ScoreImprovementService:
    ...
    def _validate_resume_keywords(
        ...
        """
        logger.info(f"Validating resume keywords for resume_id: {resume_id}")
        logger.info(f"Raw extracted_keywords field: {processed_resume.extracted_keywords}")
        
        if not processed_resume.extracted_keywords:
            logger.error(f"Resume {resume_id}: extracted_keywords field is empty/null")
            raise ResumeKeywordExtractionError(resume_id=resume_id)

        try:
            keywords_data = json.loads(processed_resume.extracted_keywords)
            logger.info(f"Parsed keywords_data: {keywords_data}, type: {type(keywords_data)}")
            
            if keywords_data is None or not isinstance(keywords_data, dict):
                logger.error(f"Resume {resume_id}: keywords_data is not a valid dict: {keywords_data}")
                raise ResumeKeywordExtractionError(resume_id=resume_id)
            keywords = keywords_data.get("extracted_keywords", [])
            logger.info(f"Extracted keywords list: {keywords}")
            
            if not keywords or len(keywords) == 0:
                logger.error(f"Resume {resume_id}: no keywords found in the list")
                raise ResumeKeywordExtractionError(resume_id=resume_id)
        except json.JSONDecodeError as e:
            logger.error(f"Resume {resume_id}: JSON decode error: {e}")
            raise ResumeKeywordExtractionError(resume_id=resume_id)
    ...
    def _validate_job_keywords(self, processed_job: ProcessedJob, job_id: str) -> None:
        ...
        """
        logger.info(f"Validating job keywords for job_id: {job_id}")
        logger.info(f"Raw extracted_keywords field: {processed_job.extracted_keywords}")
        
        if not processed_job.extracted_keywords:
            logger.error(f"Job {job_id}: extracted_keywords field is empty/null")
        ...
        try:
            ...
            if keywords_data is None or not isinstance(keywords_data, dict):
                logger.error(f"Job {job_id}: keywords_data is not a valid dict: {keywords_data}")
                raise JobKeywordExtractionError(job_id=job_id)
            keywords = keywords_data.get("extracted_keywords", [])
            logger.info(f"Extracted keywords list: {keywords}")
            
            if not keywords or len(keywords) == 0:
                logger.error(f"Job {job_id}: no keywords found in the list")
                raise JobKeywordExtractionError(job_id=job_id)
        except json.JSONDecodeError as e:
